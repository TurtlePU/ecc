...
p = int(fin.readline())
f = read_poly(fin.readline())
g = read_poly(fin.readline())
s = int(fin.readline())
...
el = ElGamal(group, g, ListEncoder(BaseEncoder(p), len(f) - 1))

try:
    cipher = list(chunks(list(map(read_poly, sys.stdin.readlines())), 2))
    assert len(cipher[-1]) == 2
except Exception:
    sys.exit(2)

try:
    decrypted = el.decrypt(s, cipher)
    assert decrypted == text
except Exception:
    sys.stderr.write("'{}'\n'{}'\n".format(text, decrypted))
    sys.exit(1)

open(sys.argv[2], 'w').write('YES\n')
