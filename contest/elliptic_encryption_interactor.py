...
k = group.pow(g, s)
print(*curve.intern(k))

n = int(inf.readline())
print(n)

inp = [ inf.readline() for _ in range(n) ]

message = ''.join(inp).rstrip('\n')
print(message)
...
def read_point(line: str):
    ...
    if line == 'Z':
        ...
        x, y = line.split(' ')
        return curve.point(int(x), int(y))

try:
    cipher = list(chunks([read_point(line) for line in sys.stdin.readlines()], 2))
except:
    sys.stderr.write('PE\n')
    sys.exit(2)

try:
    decrypted = gamal.decrypt(s, cipher)
